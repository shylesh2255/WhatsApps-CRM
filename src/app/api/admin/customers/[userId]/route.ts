...
export async function PATCH(request: Request, { params }: { params: Promise<{ userId: string }> }) {
  try {
    ...
    const { data: target } = await ctx.supabase.from('profiles').select('user_id').eq('account_id', ctx.accountId).eq('user_id', userId).maybeSingle();
    if (!target) return NextResponse.json({ error: 'Customer is not in your account' }, { status: 404 });
    const { data: updatedCustomer, error } = await supabaseAdmin().from('profiles').update(updates).eq('account_id', ctx.accountId).eq('user_id', userId).select('*').single();
    if (error) throw error;
    await ctx.supabase.from('audit_logs').insert({ account_id: ctx.accountId, actor_user_id: ctx.userId, target_user_id: userId, action: 'customer_updated', details: updates });
